...
class GoukValidator(BaseValidator):
    ...
    def compute_distance(self, src_val):
        ...
        d = []
        for true_label in tqdm(
            range(self.num_classes)
        ):  # loop over true label values, y
            l1_norm = torch.linalg.vector_norm(
                src_confmat[true_label] - src_confmat, ord=torch.inf, dim=1
            )
...
class GoukV3Validator(BaseValidator):
    ...
    def compute_distance(self, src_val):
        ...
        for true_label in tqdm(
            ...
            l1_norm = torch.linalg.vector_norm(
                src_confmat[true_label] - src_confmat, ord=1, dim=1
            )
            l1_norm[true_label] = torch.inf
...
class GoukUpdatedValidator(BaseValidator):
    ...
    def compute_domain_distance(self, src_val):
        ...
        for true_label in range(self.num_classes):  # loop over true label values, y
            for other_label in list(
                set(range(self.num_classes)) - {true_label}
            ):  # loop over other classes, apart from y
                _d = torch.norm(
                    src_confmat[true_label] - src_confmat[other_label], p=torch.inf
                )
                d.append(_d)
        return torch.stack(d).min()  # take minimum over whole set
...
if __name__ == "__main__":
    ...
    src_val = {
        ...
        "preds": torch.tensor([[2.0, -2.0], [0.5, 1.3], [0.0, 9.0], [0.1, 0.5]]),
