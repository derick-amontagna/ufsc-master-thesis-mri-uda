...
class ImprovedGoukValidator(BaseValidator):
    ...
    def compute_distance(self, src_val):
        ...
        for true_label in range(self.num_classes):  # loop over true label values, y
            for other_label in range(
                self.num_classes
            ):  # loop again over true label values, y
                l1_norm = torch.norm(
                    src_confmat[true_label] - src_confmat.mean(dim=0), p=1
                )
            d.append(
                l1_norm * src_val["labels"].argmax(dim=-1).eq(true_label).sum() / denom
            )
