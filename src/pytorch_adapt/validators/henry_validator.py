...
class HenryValidator(BaseValidator):
    ...
    def compute_distance(self, src_val, target_train):
        ...
        tgt_label_preds = (
            torch.softmax(target_train[self.layer], dim=-1)
            .argmax(dim=-1)
            .to(torch.float32)
            .to("cpu")
        )
        tgt_histogram = torch.histogram(
            tgt_label_preds, bins=self.num_classes, density=False
        ).hist / tgt_label_preds.size(0)
    ...
    def compute_score(self, src_val, target_train):
        ...
        return 1.0 - ((error + distance).item() / 2.0)
...
class HenryV3Validator(BaseValidator):
    ...
    def compute_distance(self, src_val, target_train):
        ...
        tgt_label_preds = (
            target_train[self.layer].argmax(dim=-1).to(torch.float32).to("cpu")
        )
        tgt_histogram = torch.histogram(
            tgt_label_preds, bins=self.num_classes, density=False
        ).hist / tgt_label_preds.size(0)
        ...
        W = torch.tensor(
            [
                src_val["labels"].eq(src_label).sum() / denom
                for src_label in tqdm(range(self.num_classes))
            ]
        )
...
class HenryV3RegressionValidator(BaseValidator):
    ...
    def compute_distance(self, src_val, target_train):
        kde = KDEMultivariate(
            data=target_train[self.layer].cpu().numpy(),
            var_type=["c"] * target_train[self.layer].shape[-1],
            bw="normal_reference",
        )
        ...
        s = (
            2 - 2 * kde.cdf(y_hat + eps) + 2 * kde.cdf(y_hat - eps)
        ).sum() / y_hat.shape[0]
        return s
    ...
    def compute_score(self, src_val, target_train):
        error = self.compute_error(src_val)
        distance = self.compute_distance(src_val, target_train)
        return 1.0 - (error + distance).item()
...
class HenryUpdatedValidator(BaseValidator):
    ...
    def compute_model_distance(self, src_val):
        ...
        at_init_confmat = self.confmat(
            self.src_val_at_init[self.layer].to(src_val[self.layer].device),
            self.src_val_at_init["labels"],
        ).to("cpu")
        ...
        for pred_label in range(self.num_classes):
            for src_label in range(self.num_classes):
                _d = (
                    at_init_confmat[src_label, pred_label]
                    - current_confmat[src_label, pred_label]
                ).abs()
                d.append(_d)
    ...
    def compute_score(self, src_val, target_train):
        ...
        return 1.0 - ((error + domain_distance + model_distance).item() / 3.0)
