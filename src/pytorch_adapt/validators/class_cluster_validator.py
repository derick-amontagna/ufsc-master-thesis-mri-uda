...
class ClassClusterValidator(BaseValidator):
    def __init__(
        ...
        score_fn_names=["ari", "ami", "v_measure", "fmi", "silhouette", "chi", "dbi"],
    ...
    def compute_score(self, target_train, src_train=None):
        ...
        return {
            n: self.get_clustering_performance(
                feats,
                labels,
                num_classes,
                f,
            ...
            for i, (f, t, n) in enumerate(
                zip(self.score_fns, self.score_fn_types, self.score_fn_names)
            )
        }
