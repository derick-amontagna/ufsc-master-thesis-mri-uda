...
from ..transforms.segmentation import (
    get_mnist_segmentation_transform,
    get_mnist_segmentation_label_transform,
    get_resnet_segmentation_transform,
    get_resnet_segmentation_label_transform,
)
...
def three_split_dataset(cls):
    def fn(is_training, transform_getter, **kwargs):
        transform_getter = (
            c_f.default(transform_getter, get_resnet_transform)
            if "mnist" not in kwargs["domain"]
            else get_mnist_transform
        )
        kwargs["transform"] = transform_getter(
            domain=kwargs["domain"],
            train=(kwargs["split"] in ["train", "trainval"]),
            is_training=is_training,
        )
        return cls(**kwargs)
...
def three_split_regression_dataset(cls):
    def fn(is_training, transform_getter, **kwargs):
        transform_getter = (
            c_f.default(transform_getter, get_resnet_regression_transform)
            if "mnist" not in kwargs["domain"]
            else get_mnist_transform
        )
        kwargs["transform"] = transform_getter(
...
def three_split_segmentation_dataset(cls):
    def fn(is_training, transform_getter, target_transform_getter, **kwargs):
        transform_getter = (
            c_f.default(transform_getter, get_resnet_segmentation_transform)
            if "mnist" not in kwargs["domain"]
            else get_mnist_segmentation_transform
        )
        target_transform_getter = (
            c_f.default(
                target_transform_getter, get_resnet_segmentation_label_transform
            )
            if "mnist" not in kwargs["domain"]
            else get_mnist_segmentation_label_transform
        )
        kwargs["transform"] = transform_getter(
            domain=kwargs["domain"],
            train=(kwargs["split"] in ["train", "trainval"]),
...
def get_mnist(*args, **kwargs):
    ...
    output["train"] = CombinedSourceAndTargetDataset(
        output["src_train"], output2["target_train_with_labels"]
    )
...
def get_visda2017c(*args, **kwargs):
    return get_three_split_datasets(
        three_split_dataset(VisDA2017Classification), *args, **kwargs
    )


def get_biwi(*args, **kwargs):
    return get_three_split_datasets(
        three_split_regression_dataset(BIWI), *args, **kwargs
    )


def get_mnistmr(*args, **kwargs):
    return get_three_split_datasets(
        three_split_regression_dataset(MNISTMR), *args, **kwargs
    )


def get_mnistms(*args, **kwargs):
    return get_three_split_datasets(
        three_split_segmentation_dataset(MNISTMS), *args, **kwargs
    )


def get_synthia_cityscapes(*args, **kwargs):
    return get_three_split_datasets(
        three_split_segmentation_dataset(SYNTHIACityscapes), *args, **kwargs
    )


def get_gta5_cityscapes(*args, **kwargs):
    return get_three_split_datasets(
        three_split_segmentation_dataset(GTA5Cityscapes), *args, **kwargs
    )
...
def get_dogs_and_birds(*args, **kwargs):
    return get_three_split_datasets(
        three_split_regression_dataset(DogsAndBirds), *args, **kwargs
    )


def get_animal_pose(*args, **kwargs):
    return get_three_split_datasets(
        three_split_regression_dataset(AnimalPose), *args, **kwargs
    )


def get_faces300w(*args, **kwargs):
    return get_three_split_datasets(
        three_split_regression_dataset(Faces300W), *args, **kwargs
    )
