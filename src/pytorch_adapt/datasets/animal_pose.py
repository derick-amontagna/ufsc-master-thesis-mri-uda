...
class AnimalPose(BaseDownloadableDataset):
        ...
        if self.domain == "synthetic":
            if self.split == "trainval":
                info_file = json.load(
                    open(
                        os.path.join(
                            root, "AnimalPose", "synthetic", "train", "info.json"
                        ),
                        "r",
                    )
                )
                info_file2 = json.load(
                    open(
                        os.path.join(
                            root, "AnimalPose", "synthetic", "val", "info.json"
                        ),
                        "r",
                    )
                )
                ...
                info_file = json.load(
                    open(
                        os.path.join(
                            root, "AnimalPose", "synthetic", self.split, "info.json"
                        ),
                        "r",
                    )
                )
            ...
            if self.split == "trainval":
                info_file = json.load(
                    open(
                        os.path.join(root, "AnimalPose", "real", "train", "info.json"),
                        "r",
                    )
                )
                info_file2 = json.load(
                    open(
                        os.path.join(root, "AnimalPose", "real", "val", "info.json"),
                        "r",
                    )
                )
                info_file["file_name"] += info_file2["file_name"]
                ...
                info_file = json.load(
                    open(
                        os.path.join(
                            root, "AnimalPose", "real", self.split, "info.json"
                        ),
                        "r",
                    )
                )
    ...
    def update_pose_after_crop(
        self, pose, old_width, old_height, new_width, new_height
    ):
        ...
        x_diff, y_diff = ((old_width * resizing_scale) - new_width) / 2.0, (
            (old_height * resizing_scale) - new_height
        ) / 2.0
        ...
        pose[1::2] = [
            0 if y < 0 or y > new_height or x == 0 else y
            for x, y in zip(pose[0::2], pose[1::2])
        ]
