...
class MNISTM(BaseDownloadableDataset):
    ...
    def set_paths_and_labels(self, root):
        ...
        check_length(
            self,
            {"train": 49001, "val": 10000, "trainval": 59001, "test": 9001}[self.split],
        )
    ...
    def __init__(self, root: str, split: str, transform=None, **kwargs):
        ...
        if split in ["train", "val"]:
            ...
            self.data, self.targets = [
                self.data[i] for i in split_indices[self.split]
            ], [self.targets[i] for i in split_indices[self.split]]
