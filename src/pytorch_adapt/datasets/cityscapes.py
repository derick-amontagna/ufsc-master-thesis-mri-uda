...
class Cityscapes(data.Dataset):
    def __init__(
        ...
        if DEBUG:
            print(
                "DEBUG: Cityscapes {0:} dataset path is {1:}".format(
                    self.split, self.list_path
                )
            )
        ...
        if DEBUG:
            print(
                "DEBUG: Cityscapes {0:} dataset image size is {1:}".format(
                    self.split, self.crop_size
                )
            )

        self.base_size = (
            self.base_size
            if isinstance(self.base_size, tuple)
            else (self.base_size, self.base_size)
        )
        self.crop_size = (
            self.crop_size
        ...
        try:
            ...
            print(
                "FileNotFoundError: parent of parent cwdir is "
        ...
        if DEBUG:
            print(
                "DEBUG: Cityscapes {0:} -> item_list_filepath: {1:} , first item: {2:}".format(
                    self.split, item_list_filepath, self.items[0]
                )
            )
        if DEBUG:
            print(
                "{} num images in Cityscapes {} set have been loaded.".format(
    ...
    def __getitem__(self, item):
        ...
        if item == 0 and DEBUG:
            print(
                "DEBUG: Cityscapes {0:} -> image_path: {1:}".format(
        ...
        if item == 0 and DEBUG:
            print(
                "DEBUG: Cityscapes {0:} -> gt_path: {1:}".format(
                    self.split, gt_image_path
                )
            )
        ###
    ...
    def _val_sync_transform(self, img, mask):
        if self.random_crop:
            ...
            # center crop
            w, h = img.size
            x1 = int(round((w - crop_w) / 2.0))
            y1 = int(round((h - crop_h) / 2.0))
            img = img.crop((x1, y1, x1 + crop_w, y1 + crop_h))
            mask = mask.crop((x1, y1, x1 + crop_w, y1 + crop_h))
        elif self.resize:
            img = img.resize(self.crop_size, Image.BICUBIC)
...
class City_DataLoader:
    def __init__(self, args, training=True):
        ...
        self.valid_iterations = (
            len(val_set) + self.args.batch_size
        ) // self.args.batch_size

        self.num_iterations = (
            len(data_set) + self.args.batch_size
        ) // self.args.batch_size
...
def decode_labels(mask, num_classes, num_images=1):
    ...
    return torch.from_numpy(outputs.transpose([0, 3, 1, 2]).astype("float32")).div_(
        255.0
    )
