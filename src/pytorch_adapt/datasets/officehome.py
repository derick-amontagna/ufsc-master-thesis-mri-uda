...
class OfficeHome(BaseDownloadableDataset):
    ...
    def set_paths_and_labels(self, root):
        labels_file = os.path.join(
            root, "officehome", f"{self.domain}_{self.split}.txt"
        )
        ...
        check_length(
            self,
            {
                "art": {"train": 1456, "val": 485, "trainval": 1941, "test": 486}[
                    self.split
                ],
                "clipart": {"train": 2619, "val": 873, "trainval": 3492, "test": 873}[
                    self.split
                ],
                "product": {"train": 2663, "val": 887, "trainval": 3550, "test": 889}[
                    self.split
                ],
                "real": {"train": 2614, "val": 871, "trainval": 3485, "test": 872}[
                    self.split
                ],
            }[self.domain],
        )
