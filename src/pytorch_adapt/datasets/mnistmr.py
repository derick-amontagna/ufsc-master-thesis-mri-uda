...
class MNISTMR:
    ...
    def __init__(self, root: str, domain: str, split: str, transform=None, **kwargs):
        ...
        data = pkl.load(
            open(os.path.join(root, "mnist_m_r", f"{self.domain}.pkl"), "rb")
        )
        lengths = {
            "train": 5000,
            "val": 5000,
        ...
        check_length(
            self,
            {
                "mnist": {"train": 5000, "val": 5000, "trainval": 10000, "test": 5000}[
                    self.split
                ],
                "mnistm": {"train": 5000, "val": 5000, "trainval": 10000, "test": 5000}[
                    self.split
                ],
