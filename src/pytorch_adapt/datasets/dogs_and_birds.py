...
class DogsAndBirds:
    ...
    def __init__(self, root: str, domain: str, split: str, transform=None, **kwargs):
        ...
        self.domain_info = {
            "dogs": ("StanfordDogs", "StanfordDogs/Images"),
            "birds": ("NABirds", "NABirds/nabirds/images"),
        }
        data = pkl.load(
            open(
                os.path.join(
                    root, f"{self.domain_info[self.domain][0]}/dataset_dict.pkl"
                ),
                "rb",
            )
        )
        ...
        check_length(
            self,
            {
                ...
                "dogs": {"train": 5000, "val": 5000, "trainval": 10000, "test": 5000}[
                    self.split
                ],
                "birds": {"train": 5000, "val": 5000, "trainval": 10000, "test": 5000}[
                    self.split
                ],
            }[self.domain],
        )
