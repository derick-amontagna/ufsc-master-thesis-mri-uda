...
class ClusteringHook(BaseHook):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.labeler = KMeans(
            num_classes, centroid_init=centroid_init, feat_normalizer=feat_normalizer
        )
    ...
    def call(self, inputs, losses) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        if self.with_src:
            ...
            features, logits = torch.cat(
                [src_features, target_features], dim=0
            ), torch.cat([src_logits, target_logits], dim=0)
        else:
