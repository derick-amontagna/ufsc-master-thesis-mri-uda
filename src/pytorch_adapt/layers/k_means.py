...
class KMeans(torch.nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            pseudo_labels = get_cluster_labels(
                features, labels, self.num_classes, self.centroid_init
            )
...
def get_cluster_labels(features, labels, num_classes, centroid_init):
    ...
    if centroid_init == "label_centers":
        ...
        clustering = sklearn_k_means(
            n_clusters=num_classes, init=centroids, n_init=1
        )  # replace with kmeans_pytorch (https://github.com/subhadarship/kmeans_pytorch)
